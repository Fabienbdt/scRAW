...
\begin{abstract}
Ce document d\'ecrit le fonctionnement de \textbf{scRAW} (Single-Cell Rare-population Autoencoder Weighting), un autoencoder d\'eterministe pour le clustering de donn\'ees scRNA-seq avec une attention particuli\`ere aux populations cellulaires rares. Il pr\'esente les \'etapes du pipeline : preprocessing, architecture r\'eseau, syst\`eme de pond\'eration dynamique bicrit\`ere, r\'egularisation par Triplet Loss et correction de batch adversariale (DANN) activ\'ee par d\'efaut.
\end{abstract}
...
scRAW est un \textbf{autoencoder d\'eterministe} pour le clustering de donn\'ees scRNA-seq. Dans la configuration par d\'efaut actuelle, chaque cellule produit un vecteur latent $z$ de 192 dimensions.
...
\node[stage, fill=phaseblue!10, right=0.9cm of preproc] (warmup) {\textbf{Phase 1}\\Warm-up\\74 ep.};

\node[stage, fill=phaseorange!12, right=0.9cm of warmup] (weighted) {\textbf{Phase 2}\\Weighted\\+ Triplet + DANN\\(74--209)};

\node[stage, fill=clustergreen!12, right=0.9cm of weighted] (cluster) {\textbf{Phase 3}\\HDBSCAN\\c8s8, EOM};
...
\node[minitag=phaseblue, above=0.15cm of warmup.north] {UNIFORM MSE (0--73)};
\node[minitag=phaseorange, above=0.15cm of weighted.north] {WEIGHTED MSE + DANN};
...
\draw[arr] (weighted) -- node[above, font=\tiny]{$z_{192}$} (cluster);
\draw[arr, clustergreen] (cluster) -- (out);

% (DANN activ\'e par d\'efaut, d\'emarrage \`a l'\'epoque 55)
...
\caption{Pipeline global scRAW dans sa configuration par d\'efaut actuelle. Preprocessing Scanpy, warm-up MSE uniforme, puis phase pond\'er\'ee avec Triplet Loss et DANN actif, avant clustering final HDBSCAN sur le latent space.}
...
Autoencoder sym\'etrique avec BatchNorm, LeakyReLU et Dropout. Le latent space est une pure linear projection (192D).
...
\node[layer, fill=latentcolor!25, right=1.5cm of e3, text width=1.8cm, minimum height=1.2cm,
  line width=1.5pt, draw=latentcolor] (z) {\textbf{Latent space}\\$z = 192$\\(Pure linear)};
...
\caption{Architecture scRAW. Hidden layers : Linear $\to$ BatchNorm $\to$ LeakyReLU $\to$ Dropout(0.3). Latent space : pure linear projection (192D). DANN activ\'e par d\'efaut sur la cl\'e de batch \texttt{batch}.}
\label{fig:architecture}
\end{figure}

\begin{parambox}{Param\`etres network}
\begin{tabular}{lll}
\texttt{z\_dim} & \textbf{192} & Dimension du latent space \\
...
\texttt{dropout} & \textbf{0.3} & Dropout rate \\
\end{tabular}
\end{parambox}

\subsection{Adversarial discriminator (DANN) --- activ\'e par d\'efaut}
...
\begin{tabular}{lll}
\texttt{use\_batch\_conditioning} & \textbf{true} & Activer la correction batch adversariale \\
\texttt{batch\_correction\_key} & \textbf{batch} & Colonne \texttt{obs} utilis\'ee pour le batch \\
\texttt{adversarial\_batch\_weight} & \textbf{0.0561506963} & Poids de la perte adversariale \\
\texttt{adversarial\_lambda} & \textbf{1.75} & \'Echelle du gradient reversal \\
\texttt{adversarial\_start\_epoch} & \textbf{55} & \'Epoque de d\'emarrage du DANN \\
\texttt{adversarial\_ramp\_epochs} & \textbf{60} & Dur\'ee du ramp-up lin\'eaire du DANN \\
...
\begin{importantbox}
La configuration par d\'efaut actuelle utilise \texttt{reconstruction\_distribution=mse}. Les sous-sections consacr\'ees \`a la branche NB ci-dessous documentent un mode alternatif encore disponible dans scRAW, mais non utilis\'e par le preset \texttt{default}.
\end{importantbox}
...
\end{itemize}
Plus $\theta$ est petit, plus la sur-dispersion est importante ; $\theta \to \infty$ redonne une Poisson. Dans la branche NB, la configuration par d\'efaut actuelle fixe \texttt{nb\_theta=2.5815883941220323}, mais cette valeur reste ajustable.
...
Le masquage est un \textbf{bruit d'entr\'ee stochastique} appliqu\'e pendant le warm-up, puis \textbf{conserv\'e en phase pond\'er\'ee dans la configuration par d\'efaut} (\texttt{masking\_apply\_weighted=true}). Concr\`etement, \`a \textbf{chaque mini-batch} (donc plusieurs fois par \'epoque), un nouveau masque est tir\'e ind\'ependamment pour chaque couple cellule--g\`ene ; il n'y a pas de masque fixe conserv\'e entre les \'epoques.
...
\begin{itemize}[nosep]
  \item Warm-up (\'epoques 0--73) : masquage actif, r\'e\'echantillonn\'e \`a chaque mini-batch.
  \item Phase pond\'er\'ee (\'epoques 74--209) : masquage \textbf{toujours actif} car \texttt{masking\_apply\_weighted=true}.
\end{itemize}
...
% Brace for masked
\node[font=\tiny, text=dannred] at (6.2, -1.2) {$\approx 15\%$};

\end{tikzpicture}
\caption{Masquage al\'eatoire de 15\% des g\`enes en entr\'ee. Les positions masqu\'ees (en rouge) sont remplac\'ees par 0 et pond\'er\'ees avec \texttt{masked\_recon\_weight=0.8} (positions non masqu\'ees : 0.2) dans la perte de reconstruction.}
\label{fig:masking}
\end{figure}

\begin{parambox}{Param\`etres reconstruction --- valeurs par d\'efaut}
\begin{tabular}{lll}
\texttt{reconstruction\_distribution} & \textbf{mse} & Reconstruction quadratique sur \texttt{adata.X} \\
\texttt{nb\_theta} & \textbf{2.5815883941} & Param\`etre conserv\'e pour le mode NB alternatif \\
\texttt{nb\_input\_transform} & \textbf{log1p} & Transformation par d\'efaut si la branche NB est utilis\'ee \\
\texttt{masking\_rate} & \textbf{0.15} & 15\% des g\`enes masqu\'es \\
\texttt{masked\_recon\_weight} & \textbf{0.8} & Poids relatif des positions masqu\'ees dans la perte \\
\texttt{masking\_apply\_weighted} & \textbf{true} & Masquage conserv\'e en phase 2 \\
\end{tabular}
\end{parambox}
...
% Timeline axis (scaled: 1 unit = 1 epoch, displayed up to 210)
\draw[-{Stealth}, thick] (0,0) -- (220,0) node[right, font=\small] {\'Epoques};
\foreach \x in {0,20,40,60,74,80,100,120,140,160,180,200,210} {
  \draw[thick] (\x, -0.1) -- (\x, 0.1);
  \node[below, font=\tiny] at (\x, -0.15) {\x};
}

% Phase 1: Warm-up (0-73)
\fill[phaseblue!15] (0, 0.3) rectangle (74, 2.8);
\node[font=\small\bfseries, text=phaseblue, align=center] at (37, 2.3) {Phase 1 : Warm-up};
\node[font=\tiny, align=center, text width=6.2cm] at (37, 1.5) {Perte MSE uniforme\\Masquage 15\% actif\\DANN actif d\`es l'\'epoque 55\\Poids $w_i = 1$ pour tous};

% Phase 2: Weighted (74-209)
\fill[phaseorange!12] (74, 0.3) rectangle (210, 2.8);
\node[font=\small\bfseries, text=phaseorange, align=center] at (142, 2.3) {Phase 2 : Pond\'er\'ee + Triplet + DANN};
\node[font=\tiny, align=center, text width=12cm] at (142, 1.5) {Perte MSE pond\'er\'ee : $\mathcal{L} = \sum w_i \cdot \mathcal{L}_{MSE} + \lambda_{adv}\mathcal{L}_{DANN}$\\Masquage conserv\'e \quad Poids dynamiques $w_i \in [0.45, 8.0]$};

% Triplet ramp (start epoch 84, ramp 20 ep)
\fill[tripletyellow!15] (84, -0.4) rectangle (210, -1.2);
\draw[tripletyellow!80!black, thick, -{Stealth}] (84, -0.8) -- (104, -0.8) node[midway, above, font=\tiny] {Ramp-up $T_{\text{ramp}}$ = 20 \'ep.};
\draw[tripletyellow!80!black, thick] (104, -0.8) -- (210, -0.8);
\node[font=\tiny, text=tripletyellow!80!black] at (154, -1.05) {Triplet Loss active (\texttt{rare\_triplet\_weight}=0.2346, d\`es l'\'ep. 84)};

% Weight updates markers
\foreach \e in {74,84,94,104,114,124,134,144,154,164,174,184,194,204} {
  \draw[weightpurple, thick] (\e, 0.3) -- (\e, 0.7);
  \node[font=\tiny, text=weightpurple, rotate=90, anchor=west] at (\e, 0.7) {$w$};
}

% Transition marker (epoch 74)
\draw[thick, dashed, gray] (74, -1.5) -- (74, 3.0);
\node[font=\tiny\bfseries, gray, rotate=90, anchor=south] at (74, 3.2) {Transition};

% Triplet start marker (epoch 84)
\draw[thick, dashed, tripletyellow!80!black] (84, -1.5) -- (84, 3.0);
\node[font=\tiny\bfseries, text=tripletyellow!80!black, rotate=90, anchor=south] at (84, 3.2) {Triplet start};

% Legend
\node[font=\tiny, weightpurple] at (197, 0.95) {$\uparrow$ MAJ poids};

\end{tikzpicture}%
}
\caption{Chronologie de l'entra\^inement scRAW par d\'efaut : 210 \'epoques, warm-up 74, puis phase pond\'er\'ee 136 \'epoques. Les poids dynamiques sont initialis\'es \`a la transition puis mis \`a jour tous les 10 epochs. La Triplet Loss s'active \`a l'\'epoque 84 avec un ramp-up lin\'eaire de 20 epochs.}
\label{fig:timeline}
\end{figure}

% -----------------------------------------------------------------------------
\subsection{Phase 1 : Warm-up (\'epoques 0 \`a 73)}
\label{sec:phase1}
...
\begin{tabular}{lll}
\texttt{warmup\_epochs} & \textbf{74} & Dur\'ee de la phase warm-up \\
\texttt{batch\_size} & \textbf{192} & Taille des mini-batches \\
\texttt{lr} & \textbf{0.0023367034} & Taux d'apprentissage (Adam) \\
...
\`A la fin du warm-up (\'epoque 74), l'entra\^inement s'interrompt bri\`evement pour :
\begin{enumerate}[nosep]
...
% -----------------------------------------------------------------------------
\subsection{Phase 2 : Entra\^inement pond\'er\'e (\'epoques 74 \`a 209)}
\label{sec:phase2}

\textbf{Fonction de perte totale} :
\[
  \boxed{\mathcal{L}_{total} = \underbrace{\frac{1}{N}\sum_{i=1}^{N} w_i \cdot \mathcal{L}_{recon}(x_i, \hat{x}_i)}_{\text{Reconstruction pond\'er\'ee}} \;+\; \underbrace{\rho(e) \cdot \lambda_{rare} \cdot \mathcal{L}_{triplet}}_{\text{R\'egularisation rare}} \;+\; \underbrace{\lambda_{adv} \cdot \mathcal{L}_{DANN}}_{\text{Correction batch}}}
\]
\textit{Description des termes :}
\begin{itemize}[nosep]
...
\textbf{Configuration par d\'efaut} : la correction DANN est \textbf{active} avec \texttt{adversarial\_batch\_weight=0.0561506963}, \texttt{adversarial\_start\_epoch=55} et \texttt{adversarial\_ramp\_epochs=60}. Le terme de batch s'ajoute donc \`a la reconstruction pond\'er\'ee et \`a la Triplet Loss pendant la majeure partie de la phase 2.

\textbf{Changement cl\'e} : dans la configuration par d\'efaut actuelle, le masquage reste \textbf{actif} en phase 2 (\texttt{masking\_apply\_weighted = true}), ce qui maintient le signal de d\'ebruitage pendant l'entra\^inement pond\'er\'e.

\begin{parambox}{Param\`etres entra\^inement global --- valeurs par d\'efaut}
\begin{tabular}{lll}
\texttt{epochs} & \textbf{210} & Nombre total d'\'epoques (warm-up + pond\'er\'ee) \\
\texttt{warmup\_epochs} & \textbf{74} & Dur\'ee de la phase 1 (\'epoques 0--73) \\
\texttt{batch\_size} & \textbf{192} & Cellules par mini-batch \\
\texttt{lr} & \textbf{0.0023367034} & Taux d'apprentissage Adam \\
...
\node[comp, fill=phaseorange!10, below=0.8cm of result, text width=5cm] (apply) {%
  ...
  $\mathcal{L}_{recon} = \sum w_i \cdot \mathcal{L}_{MSE}(x_i, \hat{x}_i)$};
\draw[arr, phaseorange] (result) -- (apply);

\end{tikzpicture}%
}
\caption{Syst\`eme de pond\'eration bicrit\`ere. Les deux composantes (fr\'equence de cluster et densit\'e locale) sont combin\'ees additivement via $\gamma=0.3$, puis stabilis\'ees par momentum EMA $\mu=0.85$ et clipp\'ees \`a $[0.45, 8]$.}
\label{fig:weighting}
\end{figure}
...
\texttt{weight\_exponent} ($\alpha$) & \textbf{0.7} & Exposant inverse-fr\'equence \\
...
\texttt{density\_weight\_clip} & \textbf{8.0} & Clip max densit\'e avant fusion \\
\texttt{weight\_fusion\_mode} & \textbf{additive} & $w = \gamma \cdot w_{freq} + (1-\gamma) \cdot w_{dens}$ \\
\texttt{cluster\_density\_alpha} ($\gamma$) & \textbf{0.3} & Ratio fr\'equence/densit\'e \\
\texttt{min\_cell\_weight} & \textbf{0.45} & Poids minimum apr\`es normalisation \\
\texttt{max\_cell\_weight} & \textbf{8.0} & Poids maximum \\
\texttt{dynamic\_weight\_update\_interval} & \textbf{10} & Recalcul tous les 10 cycles \\
\texttt{dynamic\_weight\_momentum} ($\mu$) & \textbf{0.85} & Inertie EMA \\
\end{tabular}
\end{parambox}
...
\node[font=\tiny, below=0.35cm of a] {Ancre ($w \geq 1.8$)};
...
\caption{Triplet Loss avec semi-hard mining. L'ancre (cellule rare, $w \geq 1.8$) est attir\'ee vers son hard positive (alli\'e le plus \'eloign\'e) et repouss\'ee de son semi-hard negative (intrus le plus proche mais d\'ej\`a au-del\`a de $d_{pos}$). La marge $m$ garantit une s\'eparation minimale.}
\label{fig:triplet}
\end{figure}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{S\'election des ancres} : seules les cellules avec $w_i \geq 1.8$ (poids de raret\'e suffisant) participent. Maximum 64 ancres par mini-batch.
\end{itemize}
...
% Phase backgrounds
\fill[phaseblue!10] (0, -0.06) rectangle (74, 1.12);
\fill[phaseorange!8] (74, -0.06) rectangle (210, 1.12);
\node[font=\small\bfseries, text=phaseblue, anchor=south] at (37, 1.12) {Warm-up (0--73)};
\node[font=\small\bfseries, text=phaseorange, anchor=south] at (142, 1.12) {Phase pond\'er\'ee (74--209)};

% Axes
\draw[-{Stealth}, thick] (-3, 0) -- (216, 0) node[right, font=\small] {\'Epoque $e$};
...
\foreach \x in {0,20,40,60,74,84,94,104,120,140,160,180,200,210} {
  \draw (\x, -0.02) -- (\x, 0.02);
  \node[below, font=\tiny] at (\x, -0.03) {\x};
}
\node[below, font=\tiny, text=tripletyellow!80!black] at (84, -0.06) {\textbf{84}};

% ρ(e) curve
% Warmup: inactive (dashed gray)
\draw[gray!50, dashed, thick] (0, 0) -- (74, 0);
% Phase 2 before triplet start: ρ=0
\draw[tripletyellow!80!black, very thick] (74, 0) -- (84, 0);
% Ramp-up: linear 84→104
\draw[tripletyellow!80!black, very thick] (84, 0) -- (104, 1);
% Plateau: ρ=1, 104→210
\draw[tripletyellow!80!black, very thick] (104, 1) -- (210, 1);

% Hatched ramp-up zone
\fill[tripletyellow!12] (84, 0) -- (104, 1) -- (104, 0) -- cycle;

% Key point dots
\fill[tripletyellow!80!black] (84,  0   ) circle (2.5pt);
\fill[tripletyellow!80!black] (89,  0.25) circle (2.5pt);
\fill[tripletyellow!80!black] (94,  0.5 ) circle (2.5pt);
\fill[tripletyellow!80!black] (99,  0.75) circle (2.5pt);
\fill[tripletyellow!80!black] (104,  1.0 ) circle (2.5pt);

% Annotations
\node[above right, font=\tiny, text=tripletyellow!80!black] at (84, 0.02)
  {$\rho{=}0$\;$\lambda_{eff}{=}0$};
\node[right, font=\tiny, text=tripletyellow!80!black] at (95, 0.5)
  {$\rho{=}0.5$\;$\lambda_{eff}{=}\lambda_{rare}/2$};
\node[above, font=\tiny, text=tripletyellow!80!black] at (104, 1.01)
  {$\rho{=}1$\;$\lambda_{eff}{=}\lambda_{rare}$};

% Dashed verticals at key epochs
\draw[dashed, tripletyellow!60] (84, 0) -- (84, -0.05);
\draw[dashed, tripletyellow!60] (104, 0) -- (104, -0.05);

% Effective lambda on right side
\draw[gray!60, -{Stealth}] (212, 0) -- (212, 1.1) node[right, font=\tiny, gray] {$\lambda_{eff}$};
\node[right, font=\tiny, gray] at (212, 0)   {0};
\node[right, font=\tiny, gray] at (212, 1.0) {$\lambda_{rare}$};

% Annotation: ramp zone
\draw[{|[scale=1.2]}-{|[scale=1.2]}, thick, tripletyellow!60] (84, -0.05) -- (104, -0.05)
  node[midway, below, font=\tiny, text=tripletyellow!80!black] {$T_{\text{ramp}}$ \'ep. de mont\'ee};

\end{tikzpicture}%
}
\caption{Profil du coefficient de ramp-up $\rho(e)$ (illustration avec valeurs par d\'efaut : $e_{\text{start}}=84$, \texttt{epochs}=210, donc $T_{\text{ramp}}=20$). Le poids effectif de la Triplet Loss dans $\mathcal{L}_{total}$ est $\rho(e)\cdot\lambda_{rare}$ : nul avant l'\'epoque de d\'emarrage, en mont\'ee lin\'eaire sur la p\'eriode de ramp-up, puis stable \`a $\lambda_{rare}$. La zone hachet\'ee repr\'esente la p\'eriode de transition.}
\label{fig:rampup}
\end{figure}

\textbf{Pourquoi un ramp-up et non une activation brutale ?} Si on activait la Triplet Loss \`a pleine force ($\lambda_{rare}$) d\`es $e_{\text{start}}$, le gradient triplet perturberait brusquement une reconstruction MSE d\'ej\`a en cours de convergence. Les triplets form\'es \`a partir de pseudo-labels encore imparfaits g\'en\`ereraient des signaux contradictoires, pouvant d\'estabiliser l'espace latent. Le ramp-up laisse au mod\`ele le temps de s'adapter progressivement au nouveau signal.
...
\texttt{rare\_triplet\_weight} ($\lambda_{rare}$) & \textbf{0.2346243650} & Poids dans la perte totale \\
\texttt{rare\_triplet\_margin} ($m$) & \textbf{0.4} & Marge de s\'eparation \\
\texttt{rare\_triplet\_min\_weight} & \textbf{1.8} & Seuil de poids pour \^etre ancre \\
\texttt{rare\_triplet\_start\_epoch} & \textbf{84} & \'Epoque d'activation absolue \\
...
Les embeddings latents $z \in \mathbb{R}^{192}$ sont extraits en fin d'entra\^inement (\texttt{cluster\_preprocess\_mode=none} : pas de r\'eduction dimensionnelle suppl\'ementaire). HDBSCAN est appliqu\'e directement sur ces 192 dimensions.
...
\node[step, fill=latentcolor!15] (embed) {\textbf{Embeddings latents}\\$z \in \mathbb{R}^{n \times 192}$\\(sortie de l'encoder)};
...
\caption{Pipeline de clustering. HDBSCAN est appliqu\'e directement sur les embeddings 192D, sans r\'eduction dimensionnelle. Les points class\'es \og bruit \fg{} ($-1$) sont r\'eassign\'es au centroid de cluster le plus proche.}
\label{fig:clustering}
\end{figure}

\textbf{Justification des param\`etres \texttt{min\_cluster\_size = 8} et \texttt{min\_samples = 8}}

\begin{itemize}[nosep]
  \item \texttt{min\_cluster\_size = 8} : impose une taille minimale un peu plus robuste pour les clusters finals tout en restant sensible aux populations rares.
  \item \texttt{min\_samples = 8} : aligne la densit\'e minimale locale sur la taille minimale de cluster, ce qui stabilise davantage la partition finale.
...
\texttt{hdbscan\_min\_cluster\_size} & \textbf{8} & Taille minimale d'un cluster \\
\texttt{hdbscan\_min\_samples} & \textbf{8} & Points cores minimum \\
...
\textbf{Signal 1 --- Reconstruction MSE pond\'er\'ee} (signal principal) : le d\'ecodeur tente de reconstruire la matrice pr\'etrait\'ee $x_{ij}$ depuis $z$. Les gradients $\nabla_z \mathcal{L}_{MSE}$ atteignent l'encoder via backpropagation. Gr\^ace aux poids $w_i$, les cellules rares g\'en\`erent une contribution plus forte au gradient, poussant l'encoder \`a mieux discriminer leur profil d'expression.

\textbf{Signal 2 --- Triplet Loss} ($\lambda_{rare}$, \`a partir de l'\'ep.~$e_{\text{start}}$) : les gradients $\nabla_z \mathcal{L}_{triplet}$ s'appliquent directement sur $z$ (pas sur le d\'ecodeur). Ils imposent une \textbf{contrainte de s\'eparation locale} : les cellules rares (ancres) doivent \^etre plus proches de leurs alli\'ees que de leurs intrus dans $\mathbb{R}^{192}$, ind\'ependamment de la qualit\'e de reconstruction.

\textbf{Interaction entre les deux signaux} : la reconstruction pond\'er\'ee agit globalement (tous les g\`enes de toutes les cellules), tandis que la Triplet Loss est locale (quelques triplets de cellules rares par mini-batch). Le param\`etre $\lambda_{rare}$ contr\^ole le rapport de force entre ces deux signaux : trop fort, la Triplet Loss d\'estabilise la reconstruction ; trop faible, elle ne structure pas assez l'espace latent. Dans la configuration par d\'efaut, le gradient DANN s'ajoute comme troisi\`eme signal \`a partir de l'\'epoque 55.
...
% Forward path
\node[module, fill=enccolor!12] (enc) {\textbf{Encodeur}\\512$\to$256$\to$128$\to$192};
\node[module, fill=latentcolor!15, right=1.5cm of enc] (z) {\textbf{$z$}\\192 dim};
\node[module, fill=deccolor!12, right=1.5cm of z] (dec) {\textbf{D\'ecodeur}\\192$\to$128$\to$256$\to$512};

% Losses
\node[loss, fill=nbblue!12, right=1.5cm of dec] (lnb) {$\mathcal{L}_{MSE}$\\pond\'er\'ee};
...
\node[draw, rounded corners=4pt, fill=white, font=\tiny, align=left, text width=5cm]
  ...
  \textcolor{nbblue}{\rule{0.5cm}{2pt}} Reconstruction MSE (principal)\\
  \textcolor{tripletyellow!80!black}{\rule{0.5cm}{2pt}} Triplet Loss (structuration locale)\\
  \textcolor{dannred}{\rule{0.5cm}{2pt}} DANN (actif par d\'efaut)};

\end{tikzpicture}%
}
\caption{Flux de gradients pendant la phase 2. Trois signaux convergent vers l'encoder : (1) la reconstruction MSE pond\'er\'ee, (2) la Triplet Loss qui structure les cellules rares dans $z$, et (3) le DANN activ\'e par d\'efaut pour la correction de batch.}
\label{fig:gradients}
\end{figure}
...
\midrule
\texttt{z\_dim} & 192 & Dimension de l'espace latent \\
...
\texttt{dropout} & 0.3 & Taux de dropout \\

\midrule
\multicolumn{3}{l}{\cellcolor{phaseblue!8}\textbf{Entra\^inement}} \\
\midrule
\texttt{epochs} & 210 & \'Epoques totales \\
\texttt{warmup\_epochs} & 74 & \'Epoques de warm-up \\
\texttt{batch\_size} & 192 & Taille des mini-batches \\
\texttt{lr} & 0.00233670337683859 & Taux d'apprentissage (Adam) \\
...
\midrule
\texttt{reconstruction\_distribution} & mse & Reconstruction quadratique \\
\texttt{nb\_theta} & 2.5815883941220323 & Dispersion NB si le mode NB est activ\'e \\
\texttt{nb\_input\_transform} & log1p & Transformation d'entr\'ee encoder \\
\texttt{masking\_rate} & 0.15 & Masquage 15\% \\
\texttt{masked\_recon\_weight} & 0.8 & Poids des positions masqu\'ees \\
\texttt{masking\_apply\_weighted} & true & Masquage actif en phase 2 \\
...
\texttt{weight\_exponent} ($\alpha$) & 0.7 & Exposant inverse-fr\'equence \\
\texttt{density\_knn\_k} & 15 & Voisins pour densit\'e locale \\
\texttt{density\_weight\_exponent} ($\beta$) & 1.0 & Exposant distance kNN \\
\texttt{density\_weight\_clip} & 8.0 & Clip max densit\'e \\
\texttt{weight\_fusion\_mode} & additive & Mode de fusion \\
\texttt{cluster\_density\_alpha} ($\gamma$) & 0.3 & Ratio fr\'equence/densit\'e \\
\texttt{min\_cell\_weight} & 0.45 & Poids minimum \\
\texttt{max\_cell\_weight} & 8.0 & Poids maximum \\
\texttt{dynamic\_weight\_update\_interval} & 10 & Recalcul tous les $N$ cycles \\
\texttt{dynamic\_weight\_momentum} ($\mu$) & 0.85 & Inertie EMA \\
...
\texttt{rare\_triplet\_weight} ($\lambda_{rare}$) & 0.2346243650039478 & Poids dans la perte totale \\
\texttt{rare\_triplet\_margin} ($m$) & 0.4 & Marge de s\'eparation \\
\texttt{rare\_triplet\_min\_weight} & 1.8 & Seuil ancre \\
\texttt{rare\_triplet\_start\_epoch} & 84 & \'Epoque d'activation \\
...
\texttt{hdbscan\_min\_cluster\_size} & 8 & Taille minimale de cluster \\
\texttt{hdbscan\_min\_samples} & 8 & Points cores minimum \\
...
\midrule
\multicolumn{3}{l}{\cellcolor{dannred!8}\textbf{Correction de batch}} \\
\midrule
\texttt{use\_batch\_conditioning} & true & Activer la correction adversariale \\
\texttt{batch\_correction\_key} & batch & Colonne \texttt{obs} utilis\'ee pour le batch \\
\texttt{adversarial\_batch\_weight} ($\lambda_{adv}$) & 0.056150696336115635 & Poids DANN \\
\texttt{adversarial\_lambda} & 1.75 & \'Echelle du gradient reversal \\
\texttt{adversarial\_start\_epoch} & 55 & \'Epoque de d\'emarrage du DANN \\
\texttt{adversarial\_ramp\_epochs} & 60 & Dur\'ee du ramp-up DANN \\
...
\node[stage, fill=phaseblue!10, below=of preproc] (warmup) {\textbf{Phase 1 : Warm-up}\\74 \'epoques (0--73)};

\node[stage, fill=phaseorange!12, below=of warmup] (weighted) {\textbf{Phase 2 : Pond\'er\'ee}\\136 \'epoques (74--209)};

\node[stage, fill=clustergreen!12, below=of weighted] (cluster) {\textbf{Phase 3 : HDBSCAN}\\sur espace latent 192D};
...
\node[detail, right=1.5cm of warmup] (warmup_d) {%
  $\bullet$ MSE uniforme ($w_i=1$), 74 \'ep.\\
  $\bullet$ Masquage 15\% actif (per mini-batch)\\
  $\bullet$ DANN activ\'e d\`es l'\'ep. 55\\
  $\bullet$ lr=0.0023367, batch=192};

\node[detail, right=1.5cm of weighted] (weighted_d) {%
  $\bullet$ MSE pond\'er\'ee : $\sum w_i \mathcal{L}_{MSE}$\\
  $\bullet$ Poids dynamiques $\Delta$10 \'ep. EMA($\mu$)\\
  $\bullet$ Triplet d\`es l'\'ep. 84 ($\lambda_{rare}=0.2346$)\\
  $\bullet$ Masquage conserv\'e en phase 2\\
  $\bullet$ DANN actif, ramp-up 60 \'ep.};

\node[detail, right=1.5cm of cluster] (cluster_d) {%
  $\bullet$ HDBSCAN c8s8, EOM\\
...
\node[detail, left=1.5cm of weighted, fill=weightpurple!5, draw=weightpurple!30, text width=3.8cm] (weight_d) {%
  ...
  $\bullet$ Momentum $\mu$, clip [0.45, 8]};
...
Dans sa configuration par d\'efaut actuelle, scRAW repose sur les composants suivants :

\begin{enumerate}[nosep]
  \item \textbf{Reconstruction} : perte MSE sur la matrice pr\'etrait\'ee, avec masquage stochastique conserv\'e pendant le warm-up puis la phase pond\'er\'ee.
  ...
  \item \textbf{Correction batch} : module DANN activ\'e par d\'efaut sur la cl\'e \texttt{batch}, avec d\'emarrage \`a l'\'epoque 55.
